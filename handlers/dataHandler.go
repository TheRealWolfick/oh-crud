...
func NewRegisterRoutes(dm *models.DataModel, mux *http.ServeMux, auth func(http.Handler) http.Handler, qm *tools.QueueManager) {
	...
	mux.Handle(fmt.Sprintf("GET /%s", *dm.End_Point), auth(newHandleGet(dm, qm)))
...
func newHandleGet(dm *models.DataModel, qm *tools.QueueManager) http.HandlerFunc {
	if dm.Allow.Get {
		return newHandleGetResource(qm, dm)
	}
	return newHandleNotAllowed(*dm.Allow)
}
...
func newHandleGetResource(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Build the query
		qm.Logger.Debug("Read overwrite select:", "value", cfg.Overwrite_Select)
		...
		qm.Logger.Debug("Loading custom with", "pointer", cfg.Custom_With)
...
func (h *DataHandler[T]) HandleGetDiff() http.HandlerFunc {
	if h.Allowed["ALL"] || h.Allowed["GET"] {
		return handleGetResource[models.Diff[T]](h.Qm, "diffs", []string{"diff_id", "diff_type", "task_id", "missing_from_supplied", "missing_from_stored", "diffs", "generated_by_user", "checksum", "created", "note", "batched", "batched_date"}, map[string]any{"diff_type": h.TableName}, h.CustomWith)
	}
	return handleNotAllowed[T](h.Allowed)
}
